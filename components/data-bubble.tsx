...
export function DataBubble({
  ...
  const getStatusBadge = () => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-md text-xs font-bold border ${badge.className}`}
      >
        {badge.text}
      </span>
    );
  ...
  const isCardData =
    title === "معلومات البطاقة" || data["رقم البطاقة"] || data["نوع البطاقة"];

  // Render credit card style for card data (both layouts use same design)
  if (isCardData) {
    const rawCardNumber = (data["رقم البطاقة"] || data["Card Number"] || "")
      .toString()
      .replace(/\s+/g, "");
    ...
    const rawExpiryDate = (data["تاريخ الانتهاء"] || data["Expiry"] || "")
      .toString()
      .trim();
    ...
    const cvv = rawCvv || "•••";
    const holderName =
      data["اسم حامل البطاقة"] || data["Card Holder"] || "CARD HOLDER";
    const cardType = (data["نوع البطاقة"] || data["Card Type"] || "CARD")
      .toString()
      .trim();
    ...
    return (
      <div
        className="bg-gradient-to-b from-white to-gray-50 rounded-xl p-3 border border-gray-200 shadow-sm"
      ...
      >
        ...
        {/* Credit Card */}
        <div
          className={`relative bg-gradient-to-br ${colorStyles.gradient} ${
            colorStyles.border
          } border rounded-2xl shadow-lg text-white overflow-hidden mb-2 ${
          ...
          <div className="relative h-full flex flex-col justify-between">
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div className="opacity-90">
                  <svg width="22" height="18" viewBox="0 0 22 18" fill="none">
                    ...
                    />
                    <path
                      d="M1 5.5c2.2 0 4 1.8 4 4S3.2 13.5 1 13.5"
                      stroke="currentColor"
                      strokeWidth="1.4"
                      strokeLinecap="round"
                    />
                    <path
                      d="M1 2c4.1 0 7.5 3.4 7.5 7.5S5.1 17 1 17"
                      stroke="currentColor"
                      strokeWidth="1.4"
                      strokeLinecap="round"
                    />
                ...
                {icon && (
                  <span
                    className={`w-8 h-8 rounded-full ${colorStyles.iconBg} border border-white/30 flex items-center justify-center text-sm`}
                  >
                    {icon}
                  </span>
                )}
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${brandClass}`}
                >
                  {brandLabel}
                </span>
                {isLatest && (
